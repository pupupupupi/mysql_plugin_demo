/*
  ...
  This class reads and writes an index file for use with the Art data 
  class. The file format is a simple binary storage of the 
  Art_index::SDE_INDEX structure. The size of the key can be set via 
  the constructor.
*/
...
Art_index::Art_index(art_tree *t, int keylen)
{
...
int Art_index::create_index(char *path, int keylen)
{
  DBUG_ENTER("Art_index::create_index");
  open_index(path);
  max_key_len = keylen;
  /* 
...
int Art_index::open_index(char *path)
{
  DBUG_ENTER("Art_index::open_index");
  /*
 ...
 */
art_node* Art_index::alloc_node(uint8_t type) {
    art_node* n;
    switch (type) {
        case NODE4:
...
// extern inline unsigned long int Art_index::art_size(art_tree *t);
// #endif

art_node** Art_index::find_child(art_node *n, uchar c) {
    int i, mask, bitfield;
    union {
        art_node4 *p1;
        art_node16 *p2;
...
// Simple inlined if
inline int Art_index::min(int a, int b) {
    return (a < b) ? a : b;
}
 ...
 * the key and node.
 */
int Art_index::check_prefix(const art_node *n, const uchar *key, int key_len, int depth) {
    int max_cmp = Art_index::min(Art_index::min(n->partial_len, MAX_PREFIX_LEN), key_len - depth);
    int idx;
...
art_leaf* Art_index::minimum(const art_node *n) {
    // Handle base cases
    if (!n) return NULL;
...
art_leaf* Art_index::maximum(const art_node *n) {
    // Handle base cases
    if (!n) return NULL;
    if (IS_LEAF(n)) return LEAF_RAW(n);
 ...
 */
art_leaf* Art_index::art_minimum(const art_tree *t) {
 ...
 */
art_leaf* Art_index::art_maximum(const art_tree *t) {
    return maximum((art_node*)t->root);
}

art_leaf* Art_index::make_leaf(const uchar *key, int key_len, long long pos) {
...
int Art_index::longest_common_prefix(art_leaf *l1, art_leaf *l2, int depth) {
    int max_cmp = Art_index::min(l1->key_len, l2->key_len) - depth;
    int idx;
    for (idx=0; idx < max_cmp; idx++) {
...
void Art_index::copy_header(art_node *dest, art_node *src) {
    ...
    memcpy(dest->partial, src->partial, Art_index::min(MAX_PREFIX_LEN, src->partial_len));
}
 ...
 */
int Art_index::prefix_mismatch(const art_node *n, const uchar *key, int key_len, int depth) {
    int max_cmp = Art_index::min(Art_index::min(MAX_PREFIX_LEN, n->partial_len), key_len - depth);
    int idx;
    for (idx=0; idx < max_cmp; idx++) {
        if (n->partial[idx] != key[depth+idx])
            return idx;
    ...
    if (n->partial_len > MAX_PREFIX_LEN) {
        ...
        max_cmp = Art_index::min(l->key_len, key_len)- depth;
        for (; idx < max_cmp; idx++) {
...
long long Art_index::recursive_insert(art_node *n, art_node **ref, const uchar *key, int key_len, long long pos, int depth, int *old, int replace) {
    ...
    if (IS_LEAF(n)) {
        ...
        memcpy(new_node->n.partial, key+depth, Art_index::min(MAX_PREFIX_LEN, longest_prefix));
    ...
    if (n->partial_len) {
        ...
        *ref = (art_node*)new_node;
        ...
        memcpy(new_node->n.partial, n->partial, Art_index::min(MAX_PREFIX_LEN, prefix_diff));
        ...
        if (n->partial_len <= MAX_PREFIX_LEN) {
            ...
            memmove(n->partial, n->partial+prefix_diff+1,
                    Art_index::min(MAX_PREFIX_LEN, n->partial_len));
        } else {
            n->partial_len -= (prefix_diff+1);
            art_leaf *l = minimum(n);
...
long long Art_index::insert_key(art_tree *t, const uchar *key, long long pos , int length)
{
  int old_val = 0;
  int key_len = length;
...
void Art_index::remove_child4(art_node4 *n, art_node **ref, art_node **l) {
    ...
    if (n->n.num_children == 1) {
        ...
        if (!IS_LEAF(child)) {
            ...
            if (prefix < MAX_PREFIX_LEN) {
                int sub_prefix = Art_index::min(child->partial_len, MAX_PREFIX_LEN - prefix);
                memcpy(n->n.partial+prefix, child->partial, sub_prefix);
                prefix += sub_prefix;
            }

            // Store the prefix in the child
            memcpy(child->partial, n->n.partial, Art_index::min(prefix, MAX_PREFIX_LEN));
...
art_leaf* Art_index::recursive_delete(art_node *n, art_node **ref, const uchar *key, int key_len, int depth) {
    // Search terminated
    if (!n) return NULL;
    ...
    if (n->partial_len) {
        ...
        if (prefix_len != Art_index::min(MAX_PREFIX_LEN, n->partial_len)) {
            return NULL;
        }
...
long long  Art_index::delete_key(art_tree *t, const uchar *key, int key_len){
...
uchar* Art_index::get_first_key(const art_tree *t) {
...
uchar* Art_index::get_last_key(const art_tree *t) {
...
long long Art_index::get_first_pos(const art_tree *t) {
    art_leaf *l = art_minimum(t);
    if (l) {
        return l->pos;
    }
...
long long Art_index::get_last_pos(const art_tree *t) {
    art_leaf *l = art_maximum(t);
    if (l) {
        return l->pos;
    }
...
art_leaf *Art_index::seek_index(const art_tree *t, const uchar *key, int key_len){
  art_node **child;
  art_node *n = t->root;
...
long long Art_index::get_index_pos(const art_tree *t, const uchar *key, int key_len){
  art_node **child;
  art_node *n = t->root;
  int prefix_len, depth = 0;
...
int Art_index::compare_leaves(const void *a, const void *b) {
    ...
    if (result == 0) {
        ...
        return leaf1->key_len - leaf2->key_len;
    }
